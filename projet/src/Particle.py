...
def update_position(self, selection_type="stochastic"):
        ...
        score_probs = np.array([len(self.problem.subsets[i]) for i in range(self.problem.m)])
        total_score = score_probs.sum()

        if total_score > 0:
            probabilities = score_probs / total_score  # Normalize the scores to create probabilities
        else:
            probabilities = np.ones_like(score_probs) / len(score_probs)

        if selection_type == "stochastic":
            # Select subsets based on the computed probabilities (binary)
            selected_indices = np.random.choice(self.problem.m, size=self.problem.k, replace=False, p=probabilities)
            self.position[:] = 0  # Reset the position
            self.position[selected_indices] = 1  # Set the selected positions to 1

        elif selection_type == "deterministic":
            # Select top k subsets with the highest probabilities
            selected_indices = np.argsort(-probabilities)[:self.problem.k]
            self.position[:] = 0  # Reset the position
            self.position[selected_indices] = 1  # Set the selected positions to 1

        elif selection_type == "standard":
            # Standard binary selection (random 1 or 0 based on probability)
            self.position = np.array([1 if random.random() < p else 0 for p in probabilities])
...
if excess > 0:
            # Randomly turn off excess bits
            self.position[np.random.choice(one_indices, size=excess, replace=False)] = 0
elif len(one_indices) < self.problem.k:
            # Add more subsets if fewer than k are selected
            zero_indices = np.where(self.position == 0)[0]
            additional = self.problem.k - len(one_indices)
            self.position[np.random.choice(zero_indices, size=additional, replace=False)] = 1
